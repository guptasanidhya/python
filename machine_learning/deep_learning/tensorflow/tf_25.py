...
def compute_gradient(x0):
	...
	with GradientTape() as tape:
		...
		y = multiply(x,x)#only multiply 2 at a time
    # Return the gradient of y with respect to x
	return tape.gradient(y,x).numpy()#y/x then +x
...
Hint
You can define a variable using Variable().
You can multiply using the multiply() operation.
Use the .gradient() method and remember that the first argument is the function whose gradient you wish to compute.
